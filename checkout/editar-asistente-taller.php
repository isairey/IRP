...
// 🔹 Obtener datos del asistente del taller
$sql = "SELECT * FROM asistentes_taller 
...
$sqlTalleres = "SELECT ID_Taller, Nombre FROM Talleres ORDER BY Nombre ASC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $nuevo_taller = $_POST['ID_Taller'];
    $nuevo_persona = $_POST['ID_Persona'];
    $nuevo_tipo = $_POST['TipoPersona'];

    // Si no se cambia el asistente → mantener tipo original
    if ($nuevo_persona == $id_persona) {
        $nuevo_tipo = $tipo;
    }

    $sqlUpdate = "UPDATE asistentes_taller 
                  SET ID_Taller = :nuevo_taller, 
                      ID_Persona = :nuevo_persona, 
                      TipoPersona = :nuevo_tipo 
                  WHERE ID_Persona = :id_persona 
                    AND TipoPersona = :tipo 
                    AND ID_Taller = :id_taller";
    ...
    $stmt->execute([
     ...
     header("Location: ../pages/ver-asistentes_taller.php?status=success");
exit();
}
?>
