...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $stmt->bindParam(4, $horaInicio);
        $stmt->bindParam(5, $horaFin);
        $stmt->bindParam(6, $lugar);
        $stmt->bindParam(7, $cupo);

        if ($stmt->execute()) {
            echo '<script>alert("Taller registrado correctamente.");</script>';
            echo '<script>window.location.href = "/ERP/ERP_IRP/pages/home.php";</script>';
        } else {
            echo "Error al registrar el taller: " . $stmt->errorInfo()[2];
        }
