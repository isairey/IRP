...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($id_asignacion && $id_seminario && $id_ponente && $fecha_asignacion) {
            $sql = "UPDATE asignacion_ponente_seminario 
                    SET ID_Seminario = :id_seminario, 
           ...
           header("Location: ../pages/ver-seminario-ponente.php?mssg=success");
exit();
        } else {
           header("Location: ../pages/ver-seminario-ponente.php?mssg=error");
exit();
        }
    }

    // Obtener ID de asignación (por GET o POST)
    ...
    $seminarios = $conn->query("SELECT ID_Seminario, Nombre FROM seminarios ORDER BY Nombre ASC")->fetchAll(PDO::FETCH_ASSOC);
...
?>
    ...
    <main>
      <div class="py-5 text-center">
        ...
        <h2>Editar Asignación Ponente-Seminario</h2>
        <p class="lead">Modifica la relación entre el seminario y su ponente.</p>
      </div>
        ...
        <div class="col-xxl-8 col-xl-8 col-lg-9">
          <form class="needs-validation" action="" method="POST" novalidate>
            ...
            <div class="row g-3">
              ...
              <!-- Selección de Seminario -->
              <div class="col-sm-12">
                <label for="ID_Seminario" class="form-label">Seminario</label>
                <select name="ID_Seminario" id="ID_Seminario" class="form-select" required>
                  <option value="">Selecciona un seminario</option>
                  <?php foreach ($seminarios as $s): ?>
                    <option value="<?= $s['ID_Seminario'] ?>" <?= $s['ID_Seminario'] == $asignacion['ID_Seminario'] ? 'selected' : '' ?>>
