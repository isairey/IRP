...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($id_taller <= 0 || $id_ponente <= 0) {
            $error = "Selecciona taller y ponente válidos.";
        } else {
            if ($accion === 'create') {
...
exit();
    ...
    if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
        ...
        $del = $conn->prepare("DELETE FROM asignacion_ponente_taller WHERE ID = :id");
...
$asigs = $conn->query("SELECT * FROM asignacion_ponentes_taller ORDER BY FechaAsignacion DESC")->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM asignacion_ponentes_taller WHERE ID = :id");
...
?>
   ...
   <div class="container">
  <h2 class="mb-4">Asignar Ponente a Taller</h2>
  ...
  <div class="card mb-4">
    <div class="card-body">
      <form method="POST" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) . (isset($_GET['edit']) ? '?edit=' . (int)$_GET['edit'] : '') ?>">
        ...
        <?php if ($editing): ?>
          <input type="hidden" name="id_asignacion" value="<?= (int)$editRecord['ID'] ?>">
        <?php endif; ?>
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <select id="id_taller" name="id_taller" class="form-select" required>
              ...
              <?php foreach ($talleresRows as $t): 
                $title = tituloTallerFromRow($t);
                $sel = ($editing && $editRecord['ID_Taller']==$t['ID_Taller']) ? 'selected' : '';
              ?>
                <option value="<?= $t['ID_Taller'] ?>" <?= $sel ?>><?= htmlspecialchars($title) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
    ...
    <table class="table table-striped table-sm">
      ...
      <tbody>
        <?php foreach ($asigs as $a): ?>
          <tr>
            <td><?= (int)$a['ID'] ?></td>
            <td><?= htmlspecialchars($talleresMap[$a['ID_Taller']] ?? ('ID ' . $a['ID_Taller'])) ?></td>
            <td><?= htmlspecialchars($ponentesMap[$a['ID_Ponente']] ?? ('ID ' . $a['ID_Ponente'])) ?></td>
            <td><?= htmlspecialchars($a['FechaAsignacion']) ?></td>
            <td>
              <a class="btn btn-sm btn-outline-primary" href="?edit=<?= $a['ID'] ?>">Editar</a>
              <a class="btn btn-sm btn-outline-danger" href="?delete=<?= $a['ID'] ?>"
                 onclick="return confirm('Eliminar asignación #<?= $a['ID'] ?>?')">Eliminar</a>
            </td>
          </tr>
        <?php endforeach; ?>
