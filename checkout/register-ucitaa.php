...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // Ejecutar la consulta
          $stmt->execute();
              header("Location: ../pages/ver-citasa.php?status=success");
exit();
    } catch (PDOException $e) {
       $conn->rollBack();
        header("Location: ../pages/ver-citasa.php?status=error&msg=" . urlencode($e->getMessage()));
exit();
    }
...
$id_usuario = isset($_GET['id_usuario']) ? urldecode($_GET['id_usuario']) : '';
$nombre_usuarioa = isset($_GET['nombre_usuario']) ? urldecode($_GET['nombre_usuario']) : '';
...
?>
    ...
    <div class="col-xxl-12 col-xxl-12">
        ...
        <form class="needs-validation" action="register-ucitaa.php" method="POST" enctype="multipart/form-data"  novalidate>
    <div class="row g-3">


    <div class="col-sm-12">
    <label for="nombre_usuario" class="form-label">Usuario</label>
    <input type="text" class="form-control" id="nombre_usuario" name="nombre_usuario" value="<?php echo $nombre_usuarioa ?>" readonly>
</div>

<input type="hidden" name="id_usuario" value="<?php echo $id_usuario; ?>">

<div class="col-sm-12">
  <label for="input_personal" class="form-label">Personal</label>
  <input type="text" id="input_personal" class="form-control" placeholder="Buscar personal...">
  <div id="sug_personal" class="list-group mt-1"></div>
  <input type="hidden" id="id_personal" name="id_personal" required>
</div>

<script>
document.addEventListener("DOMContentLoaded", () => {
  const inputPersonal = document.getElementById("input_personal");
  const sugerencias = document.getElementById("sug_personal");
  const inputHidden = document.getElementById("id_personal");
  let ultimoValor = "";

  inputPersonal.addEventListener("input", async () => {
    const q = inputPersonal.value.trim();
    ultimoValor = q;
    sugerencias.innerHTML = "";

    if (q.length < 2) return; // Esperar al menos 2 caracteres

    try {
      const response = await fetch("./ajax/buscar_personal.php?q=" + encodeURIComponent(q));
      const data = await response.json();

      // Si el texto cambió mientras esperábamos la respuesta, no mostramos nada
      if (q !== ultimoValor) return;
      ...
      data.forEach(personal => {
        ...
        item.textContent = personal.NombreCompleto;
        item.addEventListener("click", () => {
          inputPersonal.value = personal.NombreCompleto;
  ...
  document.addEventListener("click", (e) => {
    if (!e.target.closest("#input_personal") && !e.target.closest("#sug_personal")) {
      sugerencias.innerHTML = "";
    }
  });
});
