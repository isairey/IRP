...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $numModulos = $_POST["num_modulos"];

    // Los arreglos pueden no existir si numModulos = 0
    $modulos_nombres = $_POST['modulo_nombre'] ?? [];
    $modulos_descripciones = $_POST['modulo_descripcion'] ?? [];
    ...
    try {
        ...
        $stmt->execute([$nombreDiplomado, $descripcion, $numModulos, $fechaInicio, $fechaFin]);

        $diplomadoID = $conn->lastInsertId();

        // Si hay módulos, insertarlos
        if ($numModulos > 0 && !empty($modulos_nombres)) {
            $stmtModulo = $conn->prepare("INSERT INTO modulos (DiplomadoID, NombreModulo, Descripcion) VALUES (?, ?, ?)");
            for ($i = 0; $i < count($modulos_nombres); $i++) {
                $nombreModulo = trim($modulos_nombres[$i]);
                $descripcionModulo = trim($modulos_descripciones[$i] ?? '');
                if ($nombreModulo !== '') {
                    $stmtModulo->execute([$diplomadoID, $nombreModulo, $descripcionModulo]);
                }
            }
        }
...
?>
    ...
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            <label for="num_secciones" class="form-label">Número de Modulos</label>
           <input type="number" class="form-control" id="num_modulos" name="num_modulos" min="0" required>
...
function generarModulos() {
    ...
    let num = parseInt(document.getElementById("num_modulos").value);
    ...
    <div class="text-center">
      <h5 class="mb-3">Detalles de Módulos</h5>
      <p class="text-muted small">Ingresa el nombre y descripción de cada módulo.</p>
    ...
    for (let i = 0; i < num; i++) {
        html += `
          <tr>
            ...
            <td>
              <input type="text" class="form-control" name="modulo_nombre[]" required>
            </td>
            <td>
              <textarea class="form-control" name="modulo_descripcion[]" rows="2" required></textarea>
            </td>
          </tr>
        `;
    }
...
// Event listeners para generar campos dinámicamente
