...
switch ($type) {
    ...
    case 'municipio':
        ...
        // Filtro por ID de distrito
        if (!empty($_GET['distrito_id'])) {
            if (!ctype_digit((string)$_GET['distrito_id'])) {
                http_response_code(400); echo json_encode(['error' => 'distrito_id inválido']); break;
            }
        ...
        // Filtro ADICIONAL por ID de región (si se busca municipio sin saber distrito)
        else if (!empty($_GET['region_id'])) {
             if (!ctype_digit((string)$_GET['region_id'])) {
                http_response_code(400); echo json_encode(['error' => 'region_id inválido para municipio']); break;
             }
             $joins .= " JOIN distritos AS dis ON mun.id_distrito_fk = dis.id_distrito";
             $joins .= " JOIN regiones AS reg ON dis.id_region_fk = reg.id_region";
             $whereClause .= " AND reg.id_region = ?";
        ...
        $sql .= $joins . $whereClause . " ORDER BY mun.nombre LIMIT 5";
        ...
        try {
            ...
            http_response_code(500);
            echo json_encode(['error' => 'Error en consulta de municipio: ' . $e->getMessage(), 'sql' => $sql, 'params' => $queryParams]);
        }
        break;
    ...
    case 'get_full_details':
        $sql = "";
        $params_details = []; // Parámetros específicos para esta consulta
        ...
        // Determina la consulta según el ID proporcionado
        if (!empty($_GET['localidad_id'])) {
            $provided_id = $_GET['localidad_id'];
            $sql = "SELECT $selectReg, $selectDis, $selectMun, $selectLoc
                    FROM asentamientos AS asen
                    LEFT JOIN municipios AS mun ON asen.id_municipio_fk = mun.id_municipio_inegi
                    ...
                    WHERE asen.id_asentamiento = ? LIMIT 1";
        }
        else if (!empty($_GET['municipio_id'])) {
            $provided_id = $_GET['municipio_id'];
            $sql = "SELECT $selectReg, $selectDis, $selectMun
                    FROM municipios AS mun
                    LEFT JOIN distritos AS dis ON mun.id_distrito_fk = dis.id_distrito
                    LEFT JOIN regiones AS reg ON dis.id_region_fk = reg.id_region
                    WHERE mun.id_municipio_inegi = ? LIMIT 1";
        }
        else if (!empty($_GET['distrito_id'])) {
            $provided_id = $_GET['distrito_id'];
            $sql = "SELECT $selectReg, $selectDis
                    FROM distritos AS dis
                    LEFT JOIN regiones AS reg ON dis.id_region_fk = reg.id_region
                    WHERE dis.id_distrito = ? LIMIT 1";
        }
        else if (!empty($_GET['region_id'])) {
        ...
        try {
            ...
            $result = $stmt->fetch(); // fetch() devuelve false si no hay resultados

            if ($result) {
                 // Asegurar que todas las claves esperadas por JS existan (incluso si son null)
                 $response = [
                     'region_id' => $result['region_id'] ?? null,
                     'region_nombre' => $result['region_nombre'] ?? null,
                     'distrito_id' => $result['distrito_id'] ?? null,
                     'distrito_nombre' => $result['distrito_nombre'] ?? null,
                     'municipio_id' => $result['municipio_id'] ?? null,
                     'municipio_nombre' => $result['municipio_nombre'] ?? null,
                     'localidad_id' => $result['localidad_id'] ?? null,
                     'localidad_nombre' => $result['localidad_nombre'] ?? null,
                     'tipo_asentamiento' => $result['tipo_asentamiento'] ?? null,
                     'codigo_postal' => $result['codigo_postal'] ?? null,
                 ];
