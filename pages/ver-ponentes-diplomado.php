...
try {
    ...
    $sql = "
        SELECT d.ID_Diplomado, d.NombreDiplomado,
               ...
               GROUP_CONCAT(CONCAT(p.Nombre,' ',p.ApellidoPaterno,' ',p.ApellidoMaterno) SEPARATOR ', ') AS Ponentes
        FROM diplomados d
        LEFT JOIN modulos m ON m.DiplomadoID = d.ID_Diplomado
        LEFT JOIN secciones s ON s.ModuloID = m.ID_Modulo
    ...
    $sql .= " GROUP BY d.ID_Diplomado, m.ID_Modulo, s.ID
    ...
    foreach ($result as $row) {
        ...
        if ($idSec) {
            $datos[$idDipl]['Modulos'][$idMod]['Secciones'][] = [
                ...
                'Fecha' => $row['Fecha'],
                'Ponentes' => $row['Ponentes'] ?: 'Sin ponentes'
            ];
        }
    }
                ...
                <tbody>
                    <?php foreach ($datos as $dipl): ?>
                        ...
                        foreach ($dipl['Modulos'] as $mod) {
                            $rowspanDipl += max(1, count($mod['Secciones']));
                        } ?>
                        <?php $primeraFilaDipl = true; ?>
                        <?php foreach ($dipl['Modulos'] as $mod): ?>
                            <?php $secciones = $mod['Secciones'];
                            $rowspanMod = max(1, count($secciones));
                            $primeraFilaMod = true; ?>
                            <?php if ($secciones): ?>
                                <?php foreach ($secciones as $sec): ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars($sec['Ponentes']) ?></td>
                                        <td>
                                            <a href="./../checkout/editar-ponente-diplomado.php?id=<?= $sec['ID_Seccion'] ?>&idmod=<?= $mod['ID_Modulo'] ?>&iddiplo=<?= $dipl['ID_Diplomado'] ?>" class="btn btn-sm btn-warning">Editar</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    ...
                                    <td colspan="4" class="text-center text-muted">Sin secciones</td>
                                    <td>
                                        <a href="./../checkout/editar-modulos-seccion.php?idmod=<?= $mod['ID_Modulo'] ?>&id=<?= $dipl['ID_Diplomado'] ?>" class="btn btn-sm btn-info">Agregar Sección</a>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        <?php endforeach; ?>
