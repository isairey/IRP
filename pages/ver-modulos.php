...
try {
    // Contar total de diplomados con módulos filtrados
    $countSql = "SELECT COUNT(DISTINCT d.ID_Diplomado) AS total 
                 FROM diplomados d
                 INNER JOIN modulos m ON m.DiplomadoID = d.ID_Diplomado
                 WHERE d.NombreDiplomado LIKE :search";
    $countStmt = $conn->prepare($countSql);
    $countStmt->bindValue(':search', "%$search%", PDO::PARAM_STR);
    $countStmt->execute();
    $totalDiplomados = $countStmt->fetch(PDO::FETCH_ASSOC)['total'];
    $totalPaginas = ceil($totalDiplomados / $registrosPorPagina);

    // Obtener diplomados con módulos filtrados (solo página actual)
    $sql = "SELECT 
                d.ID_Diplomado,
                d.NombreDiplomado,
                COUNT(m.ID_Modulo) AS NumModulos,
                d.FechaInicio,
                d.FechaFin
            FROM diplomados d
            INNER JOIN modulos m ON m.DiplomadoID = d.ID_Diplomado
            WHERE d.NombreDiplomado LIKE :search
            GROUP BY d.ID_Diplomado
            ORDER BY d.ID_Diplomado DESC
    ...
    die("Error al obtener los diplomados: " . $e->getMessage());
}
...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Diplomados con Módulos Asignados</h1>
    </div>
    ...
    <form method="GET" class="d-flex mb-3" role="search">
        <input class="form-control me-2" type="text" placeholder="Buscar módulo..." name="search" value="<?= htmlspecialchars($search ?? '') ?>">
        <button class="btn btn-outline-success" type="submit">Buscar</button>
        <button class="btn btn-outline-secondary" type="button" onclick="window.location.href='ver-modulos.php'">
...
try {
    ...
    $countQuery = "SELECT COUNT(DISTINCT d.ID_Diplomado) FROM diplomados d
                   LEFT JOIN modulos m ON m.DiplomadoID = d.ID_Diplomado";
    ...
    $query .= " ORDER BY d.ID_Diplomado DESC LIMIT :limit OFFSET :offset";
    ...
    foreach ($diplomados as $d) {
        echo "<tr>";
        echo "<td>{$d['ID_Diplomado']}</td>";
        echo "<td>{$d['NombreDiplomado']}</td>";
        echo "<td>{$d['NumModulos']}</td>";
        echo "<td>{$d['FechaInicio']}</td>";
        echo "<td>{$d['FechaFin']}</td>";
        echo "<td>";
        echo "<a href='../checkout/editar-modulos.php?id={$d['ID_Diplomado']}' class='btn btn-primary btn-sm'><i class='bi bi-pencil-square'></i></a> ";
    ...
    // Botón Anterior
    if ($pagina > 1) {
        echo "<li class='page-item'><a class='page-link' href='?pagina=" . ($pagina - 1) . "&search=" . urlencode($_GET['search'] ?? '') . "'>&larr; Anterior</a></li>";
    } else {
    ...
    for ($i = $inicio; $i <= $fin; $i++) {
        if ($i == $pagina) {
            ...
            echo "<li class='page-item'><a class='page-link' href='?pagina=$i&search=" . urlencode($_GET['search'] ?? '') . "'>$i</a></li>";
        }
    }

    // Botón Siguiente
    if ($pagina < $totalPaginas) {
        echo "<li class='page-item'><a class='page-link' href='?pagina=" . ($pagina + 1) . "&search=" . urlencode($_GET['search'] ?? '') . "'>Siguiente &rarr;</a></li>";
    } else {
...
<?php if (isset($_GET['mensaje'])): ?>
    ...
    <script>
        Swal.fire({
            ...
            title: "<?= $_GET['mensaje'] === 'success' ? 'Modulos Asignados correctamente' : 'Error al registrar' ?>",
...
document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll('.eliminar-asignacion').forEach(button => {
        ...
        window.location.href = `./eliminar-asignacion-seminario.php?id=${donacionId}`;
                    Swal.fire({
                        icon: 'success',
                        title: '¡Eliminado!',
