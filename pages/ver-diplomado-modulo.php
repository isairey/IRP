...
try {
    ...
    // Total de diplomados
    $stmtCount = $conn->prepare("SELECT COUNT(*) FROM diplomados d $where");
    foreach ($params as $k => $v) $stmtCount->bindValue($k, $v);
    $stmtCount->execute();
    $totalRegistros = $stmtCount->fetchColumn();
    $totalPaginas = ceil($totalRegistros / $registrosPorPagina);

    // Traer diplomados con sus módulos
    $query = "
        SELECT d.ID_Diplomado, d.NombreDiplomado, d.Descripcion, d.FechaInicio, d.FechaFin,
               m.ID_Modulo, m.NombreModulo, m.Descripcion AS DescripcionModulo
        FROM diplomados d
        LEFT JOIN modulos m ON d.ID_Diplomado = m.DiplomadoID
        $where
        ORDER BY d.ID_Diplomado, m.ID_Modulo
    ...
    // Reestructurar los datos jerárquicamente
    $diplomados = [];
    foreach ($resultados as $row) {
        $idDipl = $row['ID_Diplomado'];
        if (!isset($diplomados[$idDipl])) {
            $diplomados[$idDipl] = [
                'ID_Diplomado' => $idDipl,
        ...
        if ($row['ID_Modulo']) {
            $diplomados[$idDipl]['Modulos'][] = [
                'ID_Modulo' => $row['ID_Modulo'],
                'NombreModulo' => $row['NombreModulo'],
                'DescripcionModulo' => $row['DescripcionModulo']
            ];
        }
    }
...
<!-- Tabla jerárquica de diplomados -->
 <div class="table-responsive small">
    ...
    <tbody>
      <?php foreach ($diplomados as $d): ?>
        <?php 
          $rowspan = max(count($d['Modulos']), 1);
          $printed = false;
        ?>

        <?php if (!empty($d['Modulos'])): ?>
          <?php foreach ($d['Modulos'] as $i => $mod): ?>
            <tr>
              <?php if (!$printed): ?>
                <td rowspan="<?= $rowspan ?>" class="fw-bold"><?= htmlspecialchars($d['NombreDiplomado']) ?></td>
                <td rowspan="<?= $rowspan ?>"><?= nl2br(htmlspecialchars($d['Descripcion'])) ?></td>
                <td rowspan="<?= $rowspan ?>"><?= htmlspecialchars($d['FechaInicio']) ?></td>
                <td rowspan="<?= $rowspan ?>"><?= htmlspecialchars($d['FechaFin']) ?></td>
                <?php $printed = true; ?>
              <?php endif; ?>

              <td>
                <i class="bi bi-chevron-right text-primary"></i>
                <?= htmlspecialchars($mod['NombreModulo']) ?>
                <div class="small text-muted"><?= htmlspecialchars($mod['DescripcionModulo']) ?></div>
              </td>
        ...
        <?php else: ?>
          <tr>
            <td class="fw-bold"><?= htmlspecialchars($d['NombreDiplomado']) ?></td>
...
document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll('.eliminar-diplomado').forEach(button => {
        ...
        window.location.href = `./eliminar_diplomado.php?eliminar_id=${donacionId}`;
                    Swal.fire({
                        icon: 'success',
                        title: '¡Eliminado!',
